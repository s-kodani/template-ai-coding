...
Implement
        ...
        │   Resume Protocol
        │        ↓
        │   Plan再生成 + grill-me / grilling（その前に最新 default branch を取り込み）
        │
        ↓
Verify
        ↓
...
再開時は古いImplementation Planを復元するのではなく、**最新 default branch を取り込んだうえで現在状態をもとに新しいImplementation Planを作成**し、`grill-me` / `grilling` による Plan Refinement を行います。
