...
Phase 5 では `project-verification` Skill を読み、変更種別に応じた本リポジトリ固有の検証コマンドを実行する。

### Change Intensity — 変更強度

Implementation Plan の深さと grilling の要否は、変更強度で決める。

| 強度 | 例 | Plan | grilling |
|------|-----|------|----------|
| **軽微** | typo、コメント、Skill/設定の機械的修正、テスト期待値の明示的修正 | 数行の簡易 Plan（Goal / 変更点 / 検証） | **不要** |
| **標準** | 局所機能、通常のバグ修正、既存パターン内の拡張 | 通常 Plan（全項目） | 未解決の設計判断がある場合のみ |
| **設計** | API / DB / セキュリティ / 複数境界 / ADR Level 1–2 候補 | 詳細 Plan | **必須** |

Level 1 Architecture Decision および重大な Level 2 Design Decision では、ユーザー確認を必須とする。それ以外は Agent が推奨案を採用して進めてよい。

### Plan Refinement — grill-me / grilling による計画の練り上げ

下書きの Implementation Plan を Phase 3 Decision Check または実装へ進める前に、**変更強度が「設計」**、または **「標準」で未解決の設計判断がある** 場合は `grill-me` スキルを使って計画をストレステストする。**「軽微」** の変更では grilling を省略してよい。
...
6. 「設計」強度では grilling を省略しない。「標準」では未解決判断がある場合のみ実施する。「軽微」では省略してよい。
...
- 不必要なスコープ拡大を避ける
- 実装と同時にテストを更新する
- テスト設計・観点表は `test-strategy` Skill に従う。自動化可能な振る舞いの新規追加・変更では `test-driven-development` Skill を適用する
- 新たなアーキテクチャ制約を表面化する
...
`project-verification` Skill を読み、本リポジトリの CI / Docker / トレース / Ingest 検証表に従う。テスト追加・更新時は `test-strategy` Skill で観点漏れを確認する。
