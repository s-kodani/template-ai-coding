...
3. MCP `_meta` に W3C `traceparent` + Langfuse `baggage`（`langfuse_trace_id`）
4. API キー・embedding・全文 document を span にログしない

## 自動テスト

```bash
uv run pytest tests/test_trace_propagation.py tests/test_langfuse_span_export.py tests/test_tool_trace_output.py
```
