...
2. **コード変更前に計画する** — Implementation Plan を作成する（原則ファイル化・コミットしない）。
...
5. **実態に合わせて計画を変更する** — Plan Drift を許容する。Plan は履歴文書ではない。
...
セッション終了時は Issue へ Work Checkpoint を残す。別セッション再開時は Checkpoint と Repository 現在状態を照合してから Plan を再生成する。詳細は `references/session-handoff.md`。
