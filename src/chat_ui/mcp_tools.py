...
ToolTarget = tuple[Literal["session", "unknown"], str | None]
...
def mcp_tool_name_from_llm(llm_name: str, server_id: str) -> str:
...
def prefix_tools_for_gateway(
    ...
    for tool in tools:
        ...
        if not mcp_name:
            continue
        prefixed.append({**tool, "name": llm_tool_name(server_id, str(mcp_name))})
    return prefixed
