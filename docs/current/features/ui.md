...
- ログイン継続中でも Keycloak セッションが失効していると `POST /mcp` は 403 `Keycloak セッションが失効しました。再ログインしてください` を返す。ツール実行中に失効した場合も同じ文言がツール結果として返り、チャットターンは落ちない。復旧は再ログイン（寿命の正本は [認証認可](/current/features/authentication.md)）
