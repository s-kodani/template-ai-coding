...
- PR 本文には **`Refs #<issue>`** を含める（`src/` `tests/` `scripts/` `infra/` `e2e/` `gateway/` `.apm/` 変更時は CI 必須）
  - 1 Issue に複数 PR がある場合も、各 PR は `Refs` を使う（**1:N**）
  - **`Closes #<issue>` は PR 本文に使わない** — CI が失敗する。merge 時に Issue が自動 Close し、Phase 8 前に閉じる恐れがある。Issue Close は Phase 8 後に手動で行う
